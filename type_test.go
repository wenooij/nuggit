...
func TestUnmarshalTypeInt(t *testing.T) {
	for _, tc := range []struct {
		name    string
		input   []byte
		want    Type
		...
		{name: "Type(-1)", input: []byte(`-1`), wantErr: true},
		{name: "Undefined", input: []byte(`0`), want: TypeUndefined},
		{name: "Bool", input: []byte(`1`), want: TypeBool},
		{name: "Int8", input: []byte(`2`), want: TypeInt8},
		{name: "Int16", input: []byte(`3`), want: TypeInt16},
		{name: "Int32", input: []byte(`4`), want: TypeInt32},
		{name: "Int64", input: []byte(`5`), want: TypeInt64},
		{name: "Uint8", input: []byte(`6`), want: TypeUint8},
		{name: "Uint16", input: []byte(`7`), want: TypeUint16},
		{name: "Uint32", input: []byte(`8`), want: TypeUint32},
		{name: "Uint64", input: []byte(`9`), want: TypeUint64},
		{name: "Float32", input: []byte(`10`), want: TypeFloat32},
		{name: "Float64", input: []byte(`11`), want: TypeFloat64},
		{name: "Bytes", input: []byte(`12`), want: TypeBytes},
		{name: "String", input: []byte(`13`), want: TypeString},
		{name: "Type(14)", input: []byte(`14`), wantErr: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			var got Type
			gotErr := json.Unmarshal(tc.input, &got)
			if tc.wantErr && gotErr == nil {
				t.Errorf("TestUnmarshalTypeInt(%q): wanted err, got err = nil", tc.name)
			}
			if gotErr != nil {
				t.Log(gotErr)
				if !tc.wantErr {
					t.Errorf("TestUnmarshalTypeInt(%q): wanted err = nil, got err = %v", tc.name, gotErr)
				}
			}
			if diff := cmp.Diff(tc.want, got); diff != "" {
				t.Errorf("TestUnmarshalTypeInt(%q): got diff (-want, +got):\n%v", tc.name, diff)
			}
		})
	}
}
